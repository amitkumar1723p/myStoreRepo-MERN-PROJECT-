       ...
       if(orderInfo &&cartItems &&shipping_Info){
        ...
        try {
          ...
          const { data } = await 
           ...
           axios.post(`${process.env.REACT_APP_BackendPort}/user/payment/process`, paymentData, config)
...
setAlertDetails({ AlertType: "Error", AlertMessage:result.error.message})
          ...
          if (result.paymentIntent.status = "succeeded") {
              ...
              setShowAlert(true)
              setAlertDetails({ AlertType: "Success", AlertMessage:"Submit Your Payment "})
             setpayment(true)
   ...
   setTimeout( async () => {
    ...
    try {
      ...
      setShowAlert(false)
              ...
              const { data } = await axios.post(`${process.env.REACT_APP_BackendPort}/user/createneworder`,  order, config)
              ...
              if (error.response) {
                 ...
                 setShowAlert(true)
                 setAlertDetails({ AlertType: "Error", AlertMessage:error.response.data.error})
            }
            else {
            ...
            setShowAlert(true)
            setAlertDetails({ AlertType: "Error", AlertMessage:"There 's some issue whiele processing payment"})
          }
      }
  ...
  setAlertDetails({ AlertType: "Error", AlertMessage:error.message})
        ...
        setShowAlert(true)
        setAlertDetails({ AlertType: "Error", AlertMessage:"Order Info is Not Found"})
       }
  ...
  return (
    <>
     <Helmet title='MY Store --- Process Payment' />
     {
        ...
        <CheckoutStep shippingIocnColor={"blue"} confrimOrderIconColor={"blue"}  paymentIconColor={"blue"}/> 
 ...
 <form className="paymentForm" onSubmit={Payment_Submit} >
                   <h2>Card Info</h2>
                    ...
                    <small className="cardNumber"> use This card Number <span>4000 0000 0000 5126</span> </small>
                    <div>
                    ...
                    <div>
                        ...
                        <CardCvcElement className='Payment_Input' />
                           ...
                           {   Loading==true?<Loader loading="paymentLoading"/>:  `Pay - ₹ ${ orderInfo&& orderInfo.totalPrice }`  }
       ...
       <h3> No Product In Your Cart </h3>
        <Link to="/Products"> <button className="noItemtocartButton" >View Products </button></Link>
         ...
         :  
          payment===false&&<div className="notShippingAddresh">
         <CancelIcon className="notShippingAddreshIcon" />
...
{
     payment===true&&<div className="notShippingAddresh">
